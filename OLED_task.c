...
void menu_init(void){
	...
	Menu_option *_time=submenu_creat("time",&root_menu,&show_time_hook);
	Menu_option *_IDEA=menu_option_creat("IDEA",_time,NULL);
	Menu_option *_print=menu_option_creat("print",_IDEA,NULL);
	
	Menu_option *_logo=submenu_creat("logo",_time,&show_logo_hook);
}
/**
...
//	oled_showchar(x,y+9,'.');
	...
	sscanf(RunTimeInfo,"%s %lu %s",Task_name,&t,Task_time);
	oled_showstring(row,0,(uint8_t*)Task_name);
	oled_showstring(row,13,(uint8_t*)Task_time);
	oled_showchar(row,15,'%');
	row++;
	for(uint16_t i=0;i>=399;i++)
	if(RunTimeInfo[i]=='\n'){
		char* temp=0;
		temp=RunTimeInfo+i+1;
		sscanf(temp,"%s %lu %s",Task_name,&t,Task_time);
		oled_showstring(row,0,(uint8_t*)Task_name);
		oled_showstring(row,13,(uint8_t*)Task_time);
		oled_showchar(row,15,'%');
		row++;
...
//	for(uint16_t i=0;i>=399;i++)
//	{
//		if(RunTimeInfo[i]==0)break;
//		if(RunTimeInfo[i]!='\t'&&RunTimeInfo[i]!='\%'&&RunTimeInfo[i]!='\r'&&RunTimeInfo[i]!='\n')
//		{
//			if(content_code==1)
//				Task_name[j]=RunTimeInfo[i];
//			if(content_code==2)
//				Task_time[j]=RunTimeInfo[i];
//			j++;
//		}
//		if(RunTimeInfo[i]=='\t')
//		{
